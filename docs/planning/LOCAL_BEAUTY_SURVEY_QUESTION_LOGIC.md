...
현재 구현은 `apps/mobile/src/features/local-beauty-analysis/services/localBeautySurveyScoring.ts`가 단일 소스다. 화면은 이 질문 목록을 그대로 사용하고, 결과 저장 서비스는 답변 원본과 `모르겠음`으로 답한 문항을 함께 저장한다. 각 문항은 최대 2개까지 선택할 수 있고, 저장 시 질문 id별 선택지 id 배열로 정규화한다.
...
- 일반 선택지는 한 문항에서 최대 2개까지 고를 수 있다.
- `unknown`은 단독 선택이다. `unknown`을 선택하면 기존 일반 선택지는 제거되고, 일반 선택지를 다시 고르면 `unknown`은 제거된다.
- 기존 저장 데이터처럼 질문 id에 문자열 하나만 들어온 경우에도 앱은 배열 1개로 정규화해서 읽는다.
- 2개를 선택한 문항은 각 선택지 점수를 `1 / 선택 개수` 가중치로 더한다. 예를 들어 2개를 선택하면 각 선택지는 0.5배로 반영되어 한 문항의 총 영향력이 과하게 커지지 않는다.
...
추가로 결과 레포트에는 아래 보조 분석을 계산한다.

| 분석 | 의미 |
| --- | --- |
| 컬러 세부 성향 | `season`, `depth` 점수를 바탕으로 색온도, 명도, 채도, 뉴트럴 균형 중 어떤 축이 중요한지 표시한다. |
| 상황별 분석 | `detailDailyMood`, `detailWorkMood`, `detailDateMood`, `detailPhotoMood` 계열 답변을 바탕으로 데일리, 출근, 데이트, 사진 상황별 스타일링 방향을 표시한다. |
...
| 필드 | 의미 |
| --- | --- |
| `surveyAnswers` | 전체 질문 id별 선택지 id 배열. 이후 답변 수정과 재분석에 사용한다. |
| `unknownQuestionIds` | `unknown`으로 답한 질문 id 목록. 최근 결과에서 이 문항만 다시 답할 때 사용한다. |
| `personalColor.colorAnalysis` | 색온도/명도/채도/뉴트럴 균형 중 우선 축과 각 축의 강도를 저장한다. |
| `situationAnalysis` | 데일리/출근/데이트/사진 상황별 제목, 요약, 팁을 저장한다. |
